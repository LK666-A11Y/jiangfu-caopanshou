...
exports.LIU_HAI = exports.ZI_XING = exports.XIANG_XING = exports.SAN_HUI = exports.SAN_HE = exports.LIU_HE = exports.LIU_CHONG = void 0;
...
function detectZhiRelations(zhis) {
    ...
    for (const [a, b] of pairs) {
        for (const [x, y] of exports.LIU_CHONG) {
    ...
    // 六合
    for (const [a, b] of pairs) {
        for (const [x, y] of exports.LIU_HE) {
            if ((a.zhi === x && b.zhi === y) || (a.zhi === y && b.zhi === x)) {
    ...
    // 六害
    for (const [a, b] of pairs) {
        for (const [x, y] of exports.LIU_HAI) {
            if ((a.zhi === x && b.zhi === y) || (a.zhi === y && b.zhi === x)) {
    ...
    // 相刑
    for (const xing of exports.XIANG_XING) {
    ...
    // 自刑
    for (const zx of exports.ZI_XING) {
    ...
    // 三合 / 三会 / 半合 / 拱合 / 拱会
    for (const sh of exports.SAN_HE) {
    ...
    for (const sh of exports.SAN_HUI) {
        const present = list.filter(p => sh.zhi.includes(p.zhi));
        const distinctZhi = [...new Set(present.map(p => p.zhi))];
        if (distinctZhi.length === 3) {
